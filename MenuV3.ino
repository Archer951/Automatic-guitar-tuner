...
int page = 1; // page information
bool SUflag = false; // Upwards flag
bool SlopeUp = false; // Upwards Slope flag

bool SDflag = false; // Downwards flag
bool SlopeDown = false; // Downwards Slope flag

bool SSflag = false; // Select flag
bool SlopeSelect = false; // Select Slope flag

bool checked = false; // Information for voltagemeter
int K;
int i; // Cursor information
int val; // Value of existing option on the specified page
int p; // menu stage

// Voltagemeter variables
float vin = 0.0;
float vout = 0.0;
int value = 0;
int MaxPWM = 255;
...
int pomocnicza;
float e = 0;
float PosHzTol = 0; // Upper Tolerance
float NegHzTol = 0; // Lower Tolerance
...
// ----------------------- TUNER PAGE (V1) --------------------
    ...
    if (e >= -NegHzTol && e <= PosHzTol)
    {
      ...
      delay(1000); // Delay is to show that tuner finished adjusting string frequency
      u8x8.clear();
      page = 10;
      i = 1;
      p = 2;
...
LowerLimit = DesiredHz / 1.1893; // 3 half steps lower than desired frequency
UpperLimit = DesiredHz * 1.1893; // 3 half steps higher than desired frequency
}

// ------------------- Global Interrunpt for tuner -----------------------
}
